// gcc -o sol_concurrent_threads sol_concurrent_threads.c -lpthread
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
#include <errno.h>
...
void* thread_work(void *arg) {
	unsigned long int* index = (unsigned long int*) arg;
	int i;
	for (i = 0; i < m; i++)
		*index += v;
	return NULL;
...
int main(int argc, char **argv)
{
	...
	unsigned long int* shared_array = (unsigned long int*) calloc(n, sizeof(unsigned long int));
	...
	int i;
	for (i = 0; i < n; i++)
		if (pthread_create(&threads[i], NULL, thread_work, (void *) &(shared_array[i])) != 0) {
	...
	for (i = 0; i < n; i++) {
		...
		shared_variable += shared_array[i];
	}
	printf("ok\n");

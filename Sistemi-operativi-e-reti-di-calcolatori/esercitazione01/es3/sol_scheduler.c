// gcc -o scheduler sol_scheduler.c -lpthread
...
void* client(void* arg_ptr) {
    ...
    if (sem_wait(args->semaphore)) {
        fprintf(stderr, "Can't acquire the resource, error %d\n", errno);
    }
    ...
    if (sem_post(args->semaphore)) {
        fprintf(stderr, "Can't relase the resource, error %d\n", errno);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (sem_init(semaphore, 0, NUM_RESOURCES)) {
        fprintf(stderr, "Can't create the semaphore, error %d\n", errno);
    }
    ...
    if (sem_destroy(semaphore)) {
        fprintf(stderr, "Can't destroy the semaphore, error %d\n", errno);
    }

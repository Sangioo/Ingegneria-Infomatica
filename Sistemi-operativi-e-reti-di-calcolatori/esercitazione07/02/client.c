...
int main(int argc, char* argv[]) {
    ...
    ret = sendto(socket_desc, "HELLO", sizeof("HELLO"), 0, (struct sockaddr*) &server_addr, sockaddr_len);
...
//        buf[--msg_len] = '\0'; // remove '\n' from the end of the message
        ...
        while ( bytes_sent < msg_len) {
            ret = sendto(socket_desc, buf + bytes_sent, msg_len - bytes_sent, 0, (struct sockaddr*) &server_addr, sockaddr_len);
            if (ret == -1 && errno == EINTR) continue;

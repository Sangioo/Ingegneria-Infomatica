...
int main(int argc, char* argv[]) {
    ...
    if (socket_desc < 0) handle_error("Error while creating socket");
    ...
    ret = connect(socket_desc, (struct sockaddr*) &server_addr, (socklen_t) sizeof(struct sockaddr_in));
...
//        buf[--msg_len] = '\0'; // remove '\n' from the end of the message
         ...
         */
        if (msg_len == quit_command_len && memcmp(buf, quit_command, quit_command_len) == 0) break;
        /**

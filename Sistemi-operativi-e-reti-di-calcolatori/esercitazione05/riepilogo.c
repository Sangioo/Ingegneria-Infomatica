...
sem_t *create_named_semaphore(const char *name, mode_t mode, unsigned int value) {
    ...
    if (sem == SEM_FAILED) handle_error("error while creating semafore");
...
void mainProcess() {
    ...
    data = mmap(0, sizeof(int), PROT_WRITE, MAP_SHARED, fd, 0);
    *data = 0;
...
int main(int argc, char **argv) {
    ...
    mwc = create_named_semaphore("/main_waits_children", 0666, 0);
    cwm = create_named_semaphore("/children_waits_main", 0666, 0);
    cs = create_named_semaphore("/critical_section", 0666, 1);

    shm_unlink("/my_shm");
    fd = shm_open("/my_shm", O_CREAT|O_EXCL|O_RDWR, 0666);
    ftruncate(fd, sizeof(int) * n);

    for (int i = 0; i < n; i++) {
        pid_t pid = fork();
        if (pid < 0) handle_error("error while creating children");
        if (pid == 0) {
            data = mmap(0, sizeof(int), PROT_READ, MAP_SHARED, fd, 0);
            childProcess(i);
            _exit(EXIT_SUCCESS);
        }
    }
    ...
    sem_unlink("/main_waits_children");
    sem_unlink("/children_waits_main");

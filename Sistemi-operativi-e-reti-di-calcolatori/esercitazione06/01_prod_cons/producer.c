...
void initFIFO() {
     ...
     **/
     ...
     if (ret < 0) handle_error("error while creating fifo");

     fifo = open(FIFO_NAME, O_WRONLY|O_CREAT);
     if (fifo < 0) handle_error("error while opening fifo");
     printf("ciao\n");
...
static void closeFIFO() {
     ...
     * */
     int ret = close(fifo);
     if (ret < 0) handle_error("error while closing fifo");
}
...
int writeValue(int value) {
     ...
     */
    int to_send = sizeof(int), bytes_sent = 0;
    while (to_send > 0) {
        ret = write(fifo, &value + bytes_sent, sizeof(int));
        if (ret == 0) break;
        if (ret < 0) {
            if (errno == EINTR) continue;
            handle_error("error while sending bytes");
        }
        to_send -= ret;
        bytes_sent += ret;
    }

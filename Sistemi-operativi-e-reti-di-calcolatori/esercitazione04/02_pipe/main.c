...
int write_to_pipe(int fd, const void *data, size_t data_len) {
     ...
     **/
    int r_index = 0, to_send = data_len;
    while (to_send > 0) {
        int ret = write(fd, data + r_index, to_send);
...
int read_from_pipe(int fd, void *data, size_t data_len) {
     ...
     **/
     int w_index = 0, to_recv = data_len;
     while (to_recv > 0) {
         int ret = read(fd, data + w_index, to_recv);
...
void reader(int reader_id, sem_t* read_mutex) {
     ...
     **/
    ret = close(pipefd[1]);
    if (ret) handle_error("error closing writing descriptor");
...
void writer(int writer_id, sem_t* write_mutex) {
     ...
     **/
    ret = close(pipefd[0]);
    if (ret) handle_error("error closing reading descriptor");
